...
public class DaftarKonser implements app {
    ...
    public DaftarKonser() {
        frame = new JFrame("Daftar Konser");
    ...
    public JScrollPane createMainContentPanel() {
        ...
        ArrayList<Konser> tayang = ConnectKeDB.getKonser("Sedang Tayang");
        mainContentPanel.add(createCategoryPanel("Sebentar Lagi", tayang));
        ArrayList<Konser> comingsoon = ConnectKeDB.getKonser("Coming Soon");
    ...
    private JPanel createCategoryPanel(String category, ArrayList<Konser> list) {
        ...
        JPanel KonserPanel = new JPanel();
        KonserPanel.setLayout(new BoxLayout(KonserPanel, BoxLayout.X_AXIS));
        KonserPanel.setBorder(BorderFactory.createEmptyBorder(0, 40, 20, 20));
        KonserPanel.setBackground(new Color(217,217,217,255));
        KonserPanel.add(Box.createHorizontalStrut(30));
        int i = 0;
        for (Konser kons : list) {
            ImageIcon poster = new ImageIcon(kons.getImage());
            ...
            posterLabel.addMouseListener(new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                    new DetailKonser(kons);
                }
            });
            KonserPanel.add(posterLabel);

            KonserPanel.add(Box.createHorizontalStrut(50));
            i++;
            if (i == 4) {
                break;
            }
        ...
        categoryPanel.add(KonserPanel, BorderLayout.CENTER);
